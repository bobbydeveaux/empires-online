...
class Game(Base):
    ...
    phase = Column(String(20), nullable=False, default="waiting")  # waiting, development, actions, completed
...
class GameHistory(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    game_id = Column(Integer, ForeignKey("games.id"), nullable=False)
    spawned_country_id = Column(Integer, ForeignKey("spawned_countries.id"), nullable=False)
    round_number = Column(Integer, nullable=False)
    action_type = Column(String(50), nullable=False)  # development, buy_bond, build_bank, etc.
