...
class GameLogic:
    ...
    def calculate_victory_points(spawned_country: SpawnedCountry) -> Dict[str, Any]:
        ...
        total_before_penalty = base_score + territory_bonus + stability_bonus + economic_bonus
        
        # Apply instability penalty
        instability_penalty = spawned_country.revolters > spawned_country.supporters
        final_score = total_before_penalty * 0.5 if instability_penalty else total_before_penalty
    ...
    @staticmethod
    def can_perform_action(spawned_country: SpawnedCountry, action: str, quantity: int = 1) -> bool:
    ...
    @staticmethod
    def perform_action(spawned_country: SpawnedCountry, action: str, quantity: int = 1) -> Dict[str, Any]:
        """Perform an action and return the updated state."""
        
        if not GameLogic.can_perform_action(spawned_country, action, quantity):
            return {"success": False, "error": "Cannot perform action - insufficient resources"}
