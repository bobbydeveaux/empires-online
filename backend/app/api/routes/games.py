...
from app.schemas.schemas import (
    GameCreate, Game as GameSchema, GameJoin, SpawnedCountry as SpawnedCountrySchema,
    GameState, SpawnedCountryWithDetails, DevelopmentResult, ActionResult, GameAction,
    VictoryPoints
)
...
def create_game(
    ...
    for country_name in game_data.countries:
        ...
        if not country:
            raise HTTPException(status_code=400, detail=f"Country {country_name} not found")
...
def list_games(db: Session = Depends(get_db)):
    ...
    return db.query(Game).filter(Game.phase.in_(["waiting", "development", "actions"])).all()
...
def get_game_state(
    ...
    spawned_countries = (
        ...
        .filter(SpawnedCountry.game_id == game_id)
        .all()
    )
...
def join_game(
    ...
    existing = db.query(SpawnedCountry).filter(
    ...
    existing_player = db.query(SpawnedCountry).filter(
        SpawnedCountry.game_id == game_id,
    ...
    return {"spawned_country_id": spawned_country.id, "message": "Joined game successfully"}
...
def start_game(
    ...
    if game.creator_id != current_user.id:
        raise HTTPException(status_code=403, detail="Only game creator can start the game")
    
    if game.phase != "waiting":
        raise HTTPException(status_code=400, detail="Game has already started")
    
    # Check if there are at least 2 players
    player_count = db.query(SpawnedCountry).filter(SpawnedCountry.game_id == game_id).count()
    if player_count < 2:
...
@router.post("/{game_id}/countries/{spawned_country_id}/develop", response_model=DevelopmentResult)
def execute_development(
    ...
    spawned_country = db.query(SpawnedCountry).filter(
    ...
    if not spawned_country:
        raise HTTPException(status_code=404, detail="Country not found or not owned by player")
    
    if spawned_country.development_completed:
        raise HTTPException(status_code=400, detail="Development already completed this round")
    ...
    total_players = db.query(SpawnedCountry).filter(SpawnedCountry.game_id == game_id).count()
    completed_players = db.query(SpawnedCountry).filter(
        SpawnedCountry.game_id == game_id,
...
@router.post("/{game_id}/countries/{spawned_country_id}/actions", response_model=ActionResult)
def perform_action(
    game_id: int,
    spawned_country_id: int,
    ...
    # Perform the action
    result = GameLogic.perform_action(spawned_country, action_data.action, action_data.quantity)
...
def next_round(
    ...
    if game.creator_id != current_user.id:
        raise HTTPException(status_code=403, detail="Only game creator can advance rounds")
    
    if game.phase != "actions":
        raise HTTPException(status_code=400, detail="Can only advance from actions phase")
    ...
    return {"message": f"Advanced to round {game.rounds - game.rounds_remaining + 1}", "phase": game.phase}
...
def get_leaderboard(game_id: int, db: Session = Depends(get_db)):
    ...
    spawned_countries = db.query(SpawnedCountry).filter(SpawnedCountry.game_id == game_id).all()
    leaderboard = []
